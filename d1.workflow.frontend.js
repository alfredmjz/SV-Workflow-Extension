(function (jq$) {
  ...
  if (selected_option == 4) {
    ...
    jq$('[name="programOfficeId"]')
      .closest("tr")
      .append(
        '<td id="flexcsontainer" rowspan="2"> <div id="flexbox">' +
          '<div class="user-flexchild"> </div>' +
          '<div class="assigned-flexchild"> </div>' +
          "</div></td>"
      );

    jq$(".content").css("vertical-align:middle");
  ...
  //     if (applicableOffice.includes(officeName)) {
  //       jq$(".fullWidthTable").each(function () {
  //         if ($(this).find(".shortMedium").length) {
  //           var fullName = $(this).find(".shortMedium").attr("name");
  //           console.log(
  //             fullName.substr(fullName.indexOf("["), fullName.indexOf("]"))
  //           );
  //         }
  //       });
  //     }
