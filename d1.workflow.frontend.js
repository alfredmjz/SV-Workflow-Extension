(function (jq$) {
  ...
  if (selected_office == 4) {
    ...
    jq$('[name="programOfficeId"]')
      .closest("tr")
      .append(
        '<td id="flexcontainer"> <div id="flexbox">' +
  ...
  if (applicableOffice.includes(officeName)) {
    jq$(".fullWidthTable").each(function () {
      ...
      //Insert Checkbox Holder
      jq$(
        '<td class="goldTableCell" valign="top" id="checkbox_container"></td>'
      ).prependTo(jq$(this).find("tr").filter(":nth-child(2n + 3)"));

      //Loop through all Checkbox Holder to get value and append checkbox
      var checkbox_id = $(this).find("#checkbox_container");
      jq$.each(checkbox_id, function () {
        var event_index_long = $.trim(
          $(this).nextAll().slice(0, 4).children("select").attr("name")
        );
        var event_index =
          event_index_long.substring(
            event_index_long.indexOf("[") + 1,
            event_index_long.indexOf("]")
          ) +
          " - " +
          event_index_long.substring(
            event_index_long.length - 2,
            event_index_long.length - 1
          );
        $("<input />", {
          type: "checkbox",
          name: "event",
