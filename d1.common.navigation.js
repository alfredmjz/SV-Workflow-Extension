...
(function(jq$) {
    ...
    var userinfo = jq$(".logged-in-user-text:first").text().trim().split("));")[1];
    ...
    css_bootstrap.href = chrome.extension.getURL('bootstrapiso.min.css');
    ...
    css_common.href = chrome.extension.getURL('d1.common.css');
    ...
    function nav_search() {
        ...
        jq$.ajax({
            ...
            success: function (data) {
                ...
                if (debugging) console.log('Student search success; businessObjectId: '+match[1]);
            ...
            error: function (msg) {
            	...
            	if (debugging) console.log('Student search FAILED:\n'+JSON.stringify(msg,null,4));
    ...
    function nav_menu() {
		...
		jq$("li.mainMenuTrigger div a").each(function() {
			...
			nav_shown.push($(this).attr('href').replace("/srs/navigation.do?from=",""));
		...
		var nav_html = '';
     	jq$.each(nav_objects, function(node,data) {
			...
			jq$.each(nav_objects[node][1].split(';'), function(index, value) {
				...
				if ((splitvalue.length - 1) > 1) {	//Single node, no children
					
					menu_html += '<li class="dropdown-item"><a' + target + ' href="' + link + '">'
					 + splitvalue[splitvalue.length-1].replace(/([A-Z]+)/g, ' $1').trim() + '</a></li>';
					 
				} else {							//Parent node
					
					menu_class = '';
					if (nav_objects[node][1].split(value+'.').length-1 > 0) menu_class = 'dropright';
					menu_html += '</ul></li><li class="dropdown-item dropdown-submenu ' + menu_class + '"><a' + target + ' href="' + link + '">'
					 + splitvalue[splitvalue.length-1].replace(/([A-Z]+)/g, ' $1').trim() + '</a><ul class="dropdown-menu">';
			...
			menu_html =
				'<div class="nav-item dropdown navbar-nav-button"> <!-- ' + nav_objects[node][0] + ' -->'
					+'<div class="btn-group mr-1" role="group">'
						+'<a class="btn btn-sm ' + menu_class + '" href="/srs/navigation.do?from=' + node + '">' + nav_objects[node][0] + '</a>'
						+'<div class="btn-group" role="group">'
							+'<button id="d1button' + node + '" type="button" class="btn btn-sm ' + menu_class + ' dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></button>'
							+'<ul class="dropdown-menu" style="left:-' + (nav_objects[node][0].length * 8) + 'px;">'
								+menu_html.substr(10) 
     ...
     jq$('<div id="navigation-alt" class="bootstrapiso">'
     	...
     	+'</nav>'
     	+'</div>')
     .insertBefore("#navigation01").show();
     
     
    //Create event triggers
    jq$('li.dropdown-submenu').hover(function() {
